    ...
    public static class UnityMcpSetupService
    {
            ...
            CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // Step 5: Start Server (temporary, websocket-only for health check)
                progressCallback?.Invoke(0.72f, "Starting server for health check...");
                LogStep("Starting server for health check...");
                
                ServerProcessService.SetLogBuffer(logBuffer);
                var startResult = await ServerProcessService.StartForHealthCheckAsync(settings, ct);
                ...
                LogStep($"Server started for health check (PID {startResult.ProcessId})");
                ...
                // Only stop the server if this setup run started it
                if (startResult.StartedNewProcess)
                {
                    await ServerProcessService.StopAsync(ct);
                    LogStep("Health-check server stopped");
                }
