...
const toolName = 'assets_get_data';
const toolDescription = 'Retrieves asset metadata and serialized properties for an asset by path or GUID.';
...
async function toolHandler(mcpUnity: McpUnity, params: z.infer<typeof paramsSchema>): Promise<CallToolResult> {
  ...
  if (!response.success) {
    throw new McpUnityError(
      ...
      response.message || 'Failed to retrieve asset data'
    );
  }
