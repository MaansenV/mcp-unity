        ...
        public override JObject Execute(JObject parameters)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(parentPath) || parentId.HasValue)
                {
                    ...
                    {
                        parent = McpObjectId.ToObject(parentId.Value) as GameObject;
                    }
            ...
            return new JObject
            {
                ["success"] = true,
                ["type"] = "text",
                ["message"] = $"Successfully added asset '{asset.name}' with instance ID {McpObjectId.FromObject(instance)} to the scene",
                ["instanceId"] = McpObjectId.FromObject(instance)
            };
        }
    }
}
