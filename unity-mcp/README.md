...
- run a temporary health check (server is stopped afterwards, port is free for the real MCP client)
...
├── unity-plugin/    Unity package (mirror — see below)
...
The **canonical source** for the Unity Editor package lives in:

```text
Packages/com.unity-mcp.editor/
```

The `unity-mcp/unity-plugin/` folder is a **distribution mirror** kept in sync via:

```powershell
pwsh tools/sync-unity-plugin.ps1
```

**Always edit files under `Packages/com.unity-mcp.editor/`**, then run the sync script before committing.
Do not edit `unity-mcp/unity-plugin/` directly — changes there will be overwritten.
