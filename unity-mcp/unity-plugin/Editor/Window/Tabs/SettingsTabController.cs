...
namespace UnityMCP.Editor.Window
{
    internal class SettingsTabController : TabController
    {
        ...
        public override void Build(VisualElement container)
        {
            ...
            container.Add(BuildSecurityCard());
        ...
        private VisualElement BuildSecurityCard()
        {
            var card = Card("Security (optional)", "Bearer-token auth for the WebSocket endpoint. Leave empty for local-only use.");

            card.Add(BuildFieldRow("Auth token", _settings.AuthToken, v => _settings.SetAuthToken(v), mono: true));

            var note = new Label("When set, the server requires this token in WebSocket handshakes. Leave blank for local development.");
            note.AddToClassList(WindowStyles.KvLabel);
            card.Add(note);
